        ...
        <div className="painel-flex">
                ...
                <div className="card-glass resumo">
                  ...
                  {resumo.equipes.map((equipe, index) => (
                    <div key={index} className="equipe-bloco">
                      ...
                      <ul>
                        {Object.entries(equipe.subfuncoes).map(
                          ([codigo, qtd]) => {
                            const nomeSubfuncao =
                              EQUIPES[equipe.equipe].find(
                                (f) => f.codigo === codigo
                              )?.nome || codigo;
          ...
          <div className="historico">
            {chamadasSalvas.length > 0 && (
              <section className="card-glass mt-3">
                <h3>Histórico de Requisições</h3>
                <ul>
                  {chamadasSalvas.map((item, index) => (
                    <li key={index}>
                      <strong>{item.navio}</strong> — {item.data} —{" "}
                      {item.periodo}
                    </li>
                  ))}
                </ul>
